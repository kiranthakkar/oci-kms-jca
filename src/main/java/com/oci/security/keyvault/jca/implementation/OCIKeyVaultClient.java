...
public class OCIKeyVaultClient {
    ...
    private void initialize(String certificateAuthorityId, String cryptoEndpoint) {
       	LOGGER.log(INFO, "OCIKeyVaultClient: initialize: method is invoked {0} {1}", new Object[] {certificateAuthorityId, cryptoEndpoint});
        ...
        try{
        	...
        	LOGGER.log(SEVERE, "OCI KeyVault client initialization failed {0}",e.getMessage());
        }
    ...
    public List<String> getAliases() {
        ...
        GetCertificateAuthorityRequest certAuthReq = GetCertificateAuthorityRequest.builder()
        		.certificateAuthorityId(this.certificateAuthorityId)
        		.opcRequestId("CertAuthorityAliases")
        		.build();
        CertificatesManagementClient cmc = CertificatesManagementClient.builder().build(provider);
    ...
    public Certificate getCertificate(String alias) {
    	LOGGER.log(INFO,"OCIKeyVaultClient: getCertificate: Get Certificate method is invoked for the alias {0}", alias);
        X509Certificate certificate = null;
        GetCertificateAuthorityBundleRequest certAuthBReq = GetCertificateAuthorityBundleRequest.builder()
        		.certificateAuthorityId(certificateAuthorityId)
        		.opcRequestId("GetCertAuthorityBundle")
        		.build();
        CertificatesClient cc = CertificatesClient.builder().build(provider);
        ...
        if(certAuthBRes!=null && certAuthBRes.get__httpStatusCode__()==200) {
	        ...
	        if (certificateString != null) {
	        	certificateString = certificateString.replace("-----BEGIN CERTIFICATE-----", "").replace("-----END CERTIFICATE-----", "").replace("\n", "");
	            try {
        ...
        LOGGER.log(INFO,"OCIKeyVaultClient: getCertificate: Get Certificate method is finished for the alias {0}", alias);
        cc.close();
        return certificate;
    }
    ...
    public Key getKey(String alias, char[] password) {
        ...
        if(kmsKeyID!=null) {
	        GetKeyRequest getKReq = GetKeyRequest.builder()
	        		.keyId(kmsKeyID)
	        		.opcRequestId("GetKeyForCert")
	        		.build();
	        ...
	        if(getKRes!=null && getKRes.get__httpStatusCode__()==200) {
	        	...
	        	int length = ociKey.getKeyShape().getLength().intValue();
     ...
     * @return signature
     */
    public byte[] getSignedWithPrivateKey(String digestName, String digestValue, SigningAlgorithm signAlgorithm, String keyId) {
    	LOGGER.log(INFO, "OCIKeyVaultClient: getSigned: is invoked with arguments: {0} {1} {2} {3}", new Object[] {digestName, digestValue, signAlgorithm.getValue(), keyId});
    	
    	//Get the key version for the selected key
        GetKeyRequest keyReq = GetKeyRequest.builder()
        		.keyId(keyId)
        		.build();
        KmsManagementClient kmsManagementClient = KmsManagementClient.builder().
        		endpoint(managementEndpoint).
        ...
        if(keyRes!=null && keyRes.get__httpStatusCode__()==200) {
	    	...
	    	SignDataDetails signDataDetails = SignDataDetails.builder()
	    			.keyId(keyId)
	    	...
	    	SignRequest signRequest = SignRequest.builder()
	    			...
	    			.build();
	    	KmsCryptoClient kmsCryptoClient = KmsCryptoClient.builder()
	        		.endpoint(this.cryptoEndpoint)
	        		.build(provider);
